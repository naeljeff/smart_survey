  ...
  return (
    <Surface className="w-full h-full bg-white rounded-xl" elevation={3}>
      <View className="w-full h-full bg-white rounded-xl flex flex-col items-center justify-start">
        ...
        {/* Judul */}
        <Text className="text-lg text-black font-semibold text-justify mb-3 px-3">{item.judul}</Text>
        ...
        <View className='w-full flex justify-start px-7 py-3'>
          ...
          <Text className='text-black'>{item.eff_date} <Text className='font-semibold text-black text-xs'>sampai</Text> {item.exp_date}</Text>
        </View>
      </View>
    </Surface>
  );
